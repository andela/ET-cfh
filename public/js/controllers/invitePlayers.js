...
angular.module('mean.system')
  .controller('InvitePlayersController',
    ['$scope', '$http', 'game', 'friend', 'socket',
      function ($scope, $http, game, friend, socket) {
        ...
        $scope.findUsers = function () {
          const url = $scope.searchKey.length
            ? `/api/users/findUsers/${$scope.searchKey}/${$scope.userId}`
            : '/api/users/findUsers';
          return $http.get(url,
        ...
        $scope.sendInvitation = function (user) {
          ...
          return $http.post('/api/users/invite', formData,
            { headers: { Authorization: `Bearer ${token}` } })
            .then(function (response) {
