...
angular.module('mean.system')
  .controller('FriendsController', ['$scope', '$http', 'socket',
    function ($scope, $http, socket) {
      ...
      $scope.acceptFriend = function (user) {
        ...
        $http.put(`/api/user/accept/${userId}`, { id: user._id },
          { headers: { Authorization: `Bearer ${token}` } }).then(function () {
          $scope.selectedUser = {};
          $scope.friendRequestsLength -= 1;
          $scope.friendRequests = $scope.friendRequests
      ...
      $scope.declineRequest = function (user) {
        $scope.selectedUser = user;
        $http.put(`/api/user/decline/${userId}`, { id: user._id },
          { headers: { Authorization: `Bearer ${token}` } }).then(function () {
          $scope.selectedUser = {};
          $scope.friendRequestsLength -= 1;
          $scope.friendRequests = $scope.friendRequests
      ...
      $scope.unfriend = function (user) {
        $scope.selectedUser = user;
        return $http.put(`/api/user/unfriend/${userId}`, { id: user._id },
          { headers: { Authorization: `Bearer ${token}` } }).then(function () {
      ...
      const init = function () {
        if (userId) {
          $http.get(`/api/user/getRequestCount/${userId}`,
            { headers: { Authorization: `Bearer ${token}` } })
            .then(function (response) {
